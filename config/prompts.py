...
关键要求：
- 问题必须基于提供的财务数据（证券代码、公司名称、评估维度、关键指标等字段）
- 分析过程需要清晰展示推理步骤
- 结论应该明确、可验证
...
财务数据上下文：
- 可用字段包括：证券代码、公司名称、评估维度、关键指标、财务比率、现金流数据等
...
```""",
    ...
    task_descriptions: Dict[str, str] = Field(
        default={
            ...
            "偿债能力分析": "基于资产负债表数据，分析企业的短期和长期偿债能力。关注流动比率、速动比率、资产负债率、利息保障倍数等关键指标，评估企业的债务偿还能力和财务风险。",
